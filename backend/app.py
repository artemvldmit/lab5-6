...
def create_task(user_id):
    ...
    # WebSocket — уведомляем этого пользователя на всех его устройствах
    emit_to_user(user_id, "task_created", task)
...
def update_task(user_id, task_id):
    ...
    cur.close()
    conn.close()
    # WebSocket — уведомляем этого пользователя
    emit_to_user(user_id, "task_updated", {
        "id": task_id,
...
def delete_task(user_id, task_id):
    ...
    conn.close()
    # WebSocket — уведомляем этого пользователя
    emit_to_user(user_id, "task_deleted", {"id": task_id})
    return jsonify({"message": "Deleted"}), 200
...
def complete_task(user_id, task_id):
    ...
    if row is None:
        return jsonify({"error": "Not found"}), 404
    # WebSocket — уведомляем этого пользователя
    emit_to_user(user_id, "task_updated", {"id": task_id, "done": True, "title": row[0]})
...
# Для отслеживания пользователей по сессиям
user_sessions = {}  # {user_id: set(session_ids)}
...
def on_authenticate(data):
    """Аутентифицировать пользователя на WebSocket"""
    ...
    try:
        ...
        # Запомнить пользователя для этой сессии
        if user_id not in user_sessions:
            user_sessions[user_id] = set()
        user_sessions[user_id].add(request.sid)
        print(f"User {user_id} authenticated via {request.sid}")
        emit("authenticated", {"message": "OK", "user_id": user_id})
    except jwt.ExpiredSignatureError:
...
def on_disconnect():
    """Удалить пользователя из списка при отключении"""
    print(f"Client disconnected: {request.sid}")
    # Найти и удалить из user_sessions
    for user_id in list(user_sessions.keys()):
        if request.sid in user_sessions[user_id]:
            user_sessions[user_id].remove(request.sid)
            print(f"Removed {request.sid} from user {user_id}")
...
def emit_to_user(user_id, event, data):
    """Отправить событие только пользователю"""
    if user_id in user_sessions:
        for sid in user_sessions[user_id]:
            socketio.emit(event, data, to=sid)
        print(f"Emitted '{event}' to user {user_id} ({len(user_sessions[user_id])} sessions)")
    else:
        print(f"User {user_id} has no active sessions")
